...
struct GamePath { std::string directory, filename, displayname; };
...
int main()
{
    ...
    const char* info_main, * info, * setup, * paper, * playbutton, * r_arrow, * lock, * warning, * watch, * lightbulb, * leave; int dif_info = 0, dif_warn = 0;
    {
        ...
        {
            std::ifstream from(convertToString(wSettingsPath));
            for (int i = 0; i < 50; i++) 
            {
                Sleep(10);

                if (!from.is_open()) from.open(convertToString(wSettingsPath));
                else break;
            }
            ...
            {
                std::string line;
                std::vector<std::string> lines; lines.reserve(75);
                ...
                std::ofstream to(convertToString(wSettingsPath));
                for (const std::string& text : lines) to << text << '\n';
                to.close();
            }
            SetForegroundWindow(GetConsoleWindow()); Sleep(100); std::ofstream fixTabName(convertToString(wSettingsPath), std::ios::app); fixTabName << "\n"; fixTabName.close();
        }
    }
    ...
    auto loadFile = [&]()
        {
            games.clear();
            std::ifstream file(relativePath + configFile);

            headerSize = headerSizeDefault;
            if (!file.is_open())
            {
                std::ofstream file(relativePath + configFile);
                if (!file.is_open())
                {
                    clearConsole();
                ...
                else file << "(Add all your game paths here)";
            }
            ...
            while (std::getline(file, line))
            {
                if (line.find_first_of(":") != std::string::npos)
                {
                    ...
                    size_t displaynameSplit = line.find_first_of("|");
                    std::string lowercase = line.substr(0, displaynameSplit);
                    for (char& c : lowercase)
                        if (c >= 'A' && c <= 'Z') c += 32;

                    size_t displaynameIndex = displaynameSplit != std::string::npos ? displaynameSplit + 1 + line.substr(displaynameSplit + 1).find_first_not_of(" ") : std::string::npos;
                    size_t exeIndex = lowercase.substr(0, displaynameSplit).find(".exe");
                    size_t filenameIndex = line.substr(0, exeIndex).find_last_of("\\/") + 1;
