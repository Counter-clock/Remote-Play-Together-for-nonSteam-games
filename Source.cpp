...
int main()
{
    ...
    const char* info_main, * info, * setup, * paper, * playbutton, * r_arrow, * lock, * warning, * watch, * lightbulb, * leave; int dif_info = 0, dif_warn = 0;
    {
        ...
        {
            std::ifstream from(wSettingsPath);
            for (int i = 0; i < 50; i++) 
            {
                Sleep(10);

                if (!from.is_open()) from.open(wSettingsPath);
                else break;
            }
            ...
            if (from.is_open()) 
            {
                ...
                std::ofstream to(wSettingsPath);
                for (const std::string& text : lines) to << text << '\n';
                to.close();
            }
            SetForegroundWindow(GetConsoleWindow()); Sleep(100); std::ofstream fixTabName(wSettingsPath, std::ios::app); fixTabName << "\n"; fixTabName.close();
        }
    }
    ...
    auto loadFile = [&]()
        {
            games.clear();
            std::ifstream file(wRelativePath + convertToWString(configFile));

            headerSize = headerSizeDefault;
            if (!file.is_open())
            {
                std::ofstream file(wRelativePath + convertToWString(configFile));
                if (!file.is_open())
                {
                    clearConsole();
